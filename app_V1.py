...
if st.session_state.analysis_complete and st.session_state.quantifier_results:
    ...
    # Display RIS metrics immediately above the image
    if results.get('mode') == 'RIS_segfree':
        st.markdown("### 📊 **RIS Analysis Results**")
        
        # RIS metrics in columns
    ...
    if show_circles or show_peaks or show_grid:
        ...
        if show_circles and results.get('mode') == 'RIS_segfree':
        ...
        if show_peaks and results.get('hits_xy') is not None:
            # Draw detected peaks
...
# Run analysis when button is clicked - SINGLE BUTTON ACTION!
if run_analysis:
...
if st.session_state.analysis_complete and st.session_state.quantifier_results:
    ...
    # RIS Radial Profile Graph (already displayed above in the main section)
    
    # All visualization controls are now in the main image display section above
